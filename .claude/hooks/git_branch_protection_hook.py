...
def main():
    ...
    if current_branch and current_branch in PROTECTED_BRANCHES:
        if is_git_push:
            print(f"Warning: Attempting to push directly to protected branch '{current_branch}'. Please use a feature branch and a pull request for changes.", file=sys.stderr)
            sys.exit(2) # Block the push operation
        elif is_file_modification:
            print(f"Warning: Modifying files directly on protected branch '{current_branch}'. Consider switching to a feature branch.", file=sys.stderr)
