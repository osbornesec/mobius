...
COMBINED_SENSITIVE_REGEX = re.compile(
    "|".join(f"({pattern})" for pattern, _ in SENSITIVE_PATTERNS),
...
def extract_all_conversations(transcript_path, transcript_dir):
    ...
    sessions_data = defaultdict(
        lambda: {"processed_uuids": set(), "conversation": [], "new_uuids": []}
    )
        ...
        if session_file.startswith(".last-message-"):
            ...
            try:
                ...
                with open(DEBUG_LOG_PATH, "a") as f:
                    f.write(
                        f"[{datetime.now()}] Error reading processed UUIDs from {processed_uuids_file}: {e}\n"
                    )
    ...
    try:
            ...
            for line in f:
                ...
                try:
                    ...
                    if (
                        entry_uuid
                        and entry_uuid
                        in sessions_data[entry_session_id]["processed_uuids"]
                    ):
                        continue
                    ...
                    if entry_type in ["user", "assistant"]:
                        ...
                        if text_parts:
                            ...
                            if full_text:
                                ...
                                sessions_data[entry_session_id]["conversation"].append(
                                    msg
                                )
                    ...
                    with open(DEBUG_LOG_PATH, "a") as f:
                        f.write(
                            f"[{datetime.now()}] JSON decode error in transcript: {e}\n"
                        )
                    continue
                except (OSError, IOError) as e:
                    with open(DEBUG_LOG_PATH, "a") as f:
                        f.write(
                            f"[{datetime.now()}] File error processing transcript line: {e}\n"
                        )
                    continue
                except Exception as e:
                    with open(DEBUG_LOG_PATH, "a") as f:
                        f.write(
                            f"[{datetime.now()}] Unexpected error processing transcript line: {e}\n"
                        )
                        import traceback
        ...
        with open(DEBUG_LOG_PATH, "a") as f:
            f.write(
                f"[{datetime.now()}] Error reading transcript file {transcript_path}: {e}\n"
            )

    # Convert to regular dict and return
    return {
        session_id: (data["conversation"], data["new_uuids"])
...
def main():
    ...
    with open(DEBUG_LOG_PATH, "a") as f:
        ...
        f.write(
            f"[{datetime.now()}] PYTHONPATH: {os.environ.get('PYTHONPATH', 'Not set')}\n"
        )
        ...
        f.write(
            f"[{datetime.now()}] Project config exists: {project_config.exists()}\n"
        )
    ...
    with open(DEBUG_LOG_PATH, "a") as f:
        f.write(
            f"[{datetime.now()}] tool_name='{tool_name}', session_id='{session_id}', transcript_path='{transcript_path}'\n"
        )
        ...
        if session_id and transcript_path:
            ...
            f.write(
                f"[{datetime.now()}] Session {session_id} has existing .last-message file: {has_last_message}\n"
            )
    ...
    if session_id and transcript_path:
        ...
        with open(DEBUG_LOG_PATH, "a") as f:
            f.write(
                f"[{datetime.now()}] Missing required fields: session_id='{session_id}', transcript_path='{transcript_path}'\n"
            )
        sys.exit(1)
    ...
    if not os.path.exists(transcript_path):
        with open(DEBUG_LOG_PATH, "a") as f:
            f.write(
                f"[{datetime.now()}] WARNING: Transcript file doesn't exist: {transcript_path}\n"
            )
            f.write(
                f"[{datetime.now()}] Searching for session {session_id} in all transcript files...\n"
            )
        ...
        if found_in_files:
            with open(DEBUG_LOG_PATH, "a") as f:
                f.write(
                    f"[{datetime.now()}] Found session {session_id} in: {found_in_files}\n"
                )
            ...
            with open(DEBUG_LOG_PATH, "a") as f:
                f.write(
                    f"[{datetime.now()}] Session {session_id} not found in any transcript files!\n"
                )
    ...
    if os.path.exists(transcript_path):
        ...
        try:
            ...
            try:
                ...
                sessions_conversations = extract_all_conversations(
                    transcript_path, transcript_dir
                )

                with open(DEBUG_LOG_PATH, "a") as f:
                    f.write(
                        f"[{datetime.now()}] Found {len(sessions_conversations)} sessions in transcript\n"
                    )

                # Process each session's conversation and update its .last-message file
                for session_id_found, (
                    conversation,
                    new_uuids,
                ) in sessions_conversations.items():
                    session_last_message_file = (
                        f"{transcript_dir}/.last-message-{session_id_found}"
                    )
                    ...
                    if not os.path.exists(session_last_message_file):
                        with open(DEBUG_LOG_PATH, "a") as f:
                            f.write(
                                f"[{datetime.now()}] Creating new .last-message file: {session_last_message_file}\n"
                            )
                    ...
                    if new_uuids:
                        with open(DEBUG_LOG_PATH, "a") as f:
                            f.write(
                                f"[{datetime.now()}] Adding {len(new_uuids)} new UUIDs for session {session_id_found}\n"
                            )
