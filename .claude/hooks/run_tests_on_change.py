...
def main():
    ...
    try:
        ...
        process = subprocess.run(
            test_command, capture_output=True, text=True, check=False
        )

        if process.returncode != 0:
            print(
                f"Tests failed for {file_path}:\n{process.stdout}\n{process.stderr}",
                file=sys.stderr,
            )
        else:
        ...
        print(
            "Error: Test command 'npm' not found. Please ensure Node.js and npm are installed.",
            file=sys.stderr,
        )
