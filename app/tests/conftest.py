...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(name in item.nodeid for name in ["test_batch_operations", "test_index_creation"]):
...
def check_environment():
    ...
    if missing_required:
        pytest.fail(f"Required environment variables not set: {', '.join(missing_required)}")
