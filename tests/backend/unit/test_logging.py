...
from app.core.logging import LogConfig, SensitiveDataMasker, setup_logging, get_logger
...
class TestProductionCodeImports:
    ...
    def test_no_local_reimplementations(self):
        ...
        test_class_names = {name for name, _ in test_classes}
        
        conflicts = production_names.intersection(test_class_names)
        ...
        test_function_names = {name for name, _ in test_members}
        
        conflicts = production_function_names.intersection(test_function_names)
...
class TestRequestResponseLogging:
    ...
    def test_error_response_logging(self, app, log_config, clean_logging, capture_logs):
        ...
        # Check for error log
        error_log = next(
            (log for log in capture_logs if log.get("event") == "request_failed"),
            None
        )
...
class TestExceptionHandling:
    ...
    def test_production_exception_without_stack_trace(self, log_config, clean_logging, capture_logs):
        ...
        assert "exception" in log_entry
        # Check that we only have the first line (no full traceback)
